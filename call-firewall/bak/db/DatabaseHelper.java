...
import org.onetwo.android.app.callfirewall.CallFirewallFactory.Destroyable;
...
public class DatabaseHelper extends SQLiteOpenHelper implements Destroyable {
	...
	public Cursor find(String table, Object...args){
		...
		if(ObjectUtils.isEmpty(args)){
			cursor = this.getDatabase().query(table, t.getColumnsArray(), null, null, null, null, t.getId().getName());
			return cursor;
		}
		...
		if(StringUtils.hasText(where))
			cursor = this.getDatabase().query(table, t.getColumnsArray(), where, (String[])values.toArray(new String[values.size()]), null, null, t.getId().getName());
		else
			cursor = this.getDatabase().query(table, t.getColumnsArray(), null, null, null, null, t.getId().getName());
		return cursor;
	}
	...
	public Cursor findById(String table, Object id){
		Table t = getTable(table);
		Cursor cursor = this.getDatabase().query(table, t.getColumnsArray(), t.getId().getName()+"=?", new String[]{id.toString()}, null, null, null, null);
		if(cursor!=null)
			cursor.moveToFirst();
		return cursor;
	}
	...
	public long count(String table, Object...fields){
		...
		if(fields==null){
			...
			String where = this.sqlSymbol.createHql(properties, values);
			if(StringUtils.hasText(where)){
				cursor = getDatabase().rawQuery(MyUtils.append(sql, " ", where), (String[])values.toArray(new String[values.size()]));
			}else
				cursor = getDatabase().rawQuery(sql, null);
		}
		int count = 0;
